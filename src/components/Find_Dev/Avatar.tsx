...
const Avatars = ({ image, name, avali }: AvatarsProps) => {

    
    
    const result = GetAvaliableById(avali);
    const getbyidavaliable = result && 'getbyidavaliable' in result ? result.getbyidavaliable : undefined;


    const isPostedByCurrent = avali?.includes(getbyidavaliable?.postedBy);
    const availableValue = isPostedByCurrent ? getbyidavaliable?.available : undefined;
  ...
  return (
    <div
      className={`relative w-16 h-16 rounded-full ${
        availableValue ? "border-3 border-orange-600" : ""
      } flex items-center justify-center`}
    >
      <div className="w-full h-full rounded-full overflow-hidden">
        <Avatar className="w-full h-full bg-zinc-300">
          <AvatarImage
            src={image}
            alt="avatar"
